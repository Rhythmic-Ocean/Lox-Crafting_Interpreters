...
int getAt(Chunk *chunk, int offset) {
  ...
  return chunk->lines[chunk->LineIndex - 1]; // if the loop ends and the valid
                                             // value still can't be found then
  // that implies
}
...
int disassembleInstruction(Chunk *chunk, int offset) {
  ...
  if (offset > 0 &&
      getAt(chunk, offset) ==
          getAt(chunk, offset - 1)) { // if a source code comes from the same
                                      // line as the preceeding line, we do this
    printf(" |");
  } else {
  ...
  case OP_GET_GLOBAL:
    return constantInstruction(
        "OP_GET_GLOBAL", chunk,
        offset); // printing OP_GET_GLOBAL + string of the var name
