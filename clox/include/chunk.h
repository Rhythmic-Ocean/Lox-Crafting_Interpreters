...
typedef struct {    // a dynamic array wrapper
  ...
  int LineIndex;  // current index of lines, it's number of elements in the line
                  // acutally, LineCount would be more accurate...
  uint8_t *code;  // bytecodes array
  int *lines;     // line number for bytecodes
  int *new_lines; // indexes of bytecode in uint8_t code where new line starts.
                  // Implemented under Chp14, challenge 1's directive
...
When new elemented is to be added but not enough capacity:
    allocate new array with more capacity and copy the existing to a new one and
delete the old one and update the code to point to new array
*/
