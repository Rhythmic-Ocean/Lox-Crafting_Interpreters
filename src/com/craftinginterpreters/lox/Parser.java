...
class Parser{
    ...
    private Stmt varDeclaration(){//this one is just for var declaration, the other match(IDENTIFIER) is for other usages like printing or other assigning of variables
        Token name = consume(IDENTIFIER, "Expect variable name.");

        Expr initializer = null;
        if(match(EQUAL)){
            initializer = expression();//should be a valid expression otherwise it error will be thrown at primary
        }
    ...
    private Expr assignment(){
        ...
        if(match(EQUAL)){
            ...
            if(expr instanceof Expr.Variable){
                Token name = ((Expr.Variable)expr).name;//despite already checking for Expr being of Variable class, java needs explicit typecast to access vars inside of the object
                return new Expr.Assign(name, value);
            }
            error(equals, "Invalid assignment target.");//sends an error of the left side is not of variable type
            //even a+b  a+b is r value will be Expr.Binary
            //(a) = 3 will also be error cuz (a) is Expr.Grouping
            //also the error is not thrown cuz the parser knows exactly what's the problem so need to go to panic mode
        }
    ...
    private Token consume(TokenType type, String message){
        ...
        throw error(peek(), message);//after we get an error, we throw it and in the catch we try discarding the entire statement until we reach it's end.
        //and starting next statement we do normal parsing
        //but for now we just panic and unwind all the way to the top as we don't have statements yet
    }
    ...
    private void synchronize(){
        ...
        while(!isAtEnd()){
            ...
            switch(peek().type){///this too cuz either of these spots in cases is a good place for the parser to resume it's parsing.
                case CLASS:
                case FUN:
                case VAR:
                case FOR:
